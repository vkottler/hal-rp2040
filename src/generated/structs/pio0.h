...
but are also exposed here. */
   ...
   instruction immediately (including jumps) and then resume execution. */
     ...
     * the following are cleared: input and output shift counters; the contents
     * of the input shift register; the delay counter; the waiting-on-IRQ
     * state; any stalled instruction written to SMx_INSTR or run by OUT/MOV
     * EXEC; any pin write left asserted due to OUT_STICKY.\n\n The program
     * counter, the contents of the output shift register and the X/Y scratch
     ...
     * that setting/clearing SM_ENABLE does not stop the clock divider from
     * running, so once multiple state machines' clocks are synchronised, it is
     * safe to disable/reenable a state machine, whilst keeping the clock
     * dividers in sync.\n\n Note also that CLKDIV_RESTART can be written to
     * whilst the state machine is running, and this is useful to resynchronise
     * clock dividers after the divisors (SMx_CLKDIV) have been changed
     * on-the-fly.
     */
