/**
 * \file
 * \brief Generated by ifgen (3.1.6).
 */
#pragma once

#include <cstdint>
#include <cstring>

namespace RP2040
{

enum class I2C0_IC_CON_IC_10BITADDR_SLAVE : uint8_t
{
    _7BITS /*!< Slave 7Bit addressing */,
    _10BITS = 1 /*!< Slave 10Bit addressing */
};
static_assert(sizeof(I2C0_IC_CON_IC_10BITADDR_SLAVE) == 1);

/**
 * Converts I2C0_IC_CON_IC_10BITADDR_SLAVE to a C string.
 *
 * \param[in] instance Value to convert.
 * \return             A C string representation of the value.
 */
inline const char *to_string(I2C0_IC_CON_IC_10BITADDR_SLAVE instance)
{
    const char *result = "UNKNOWN I2C0_IC_CON_IC_10BITADDR_SLAVE";

    switch (instance)
    {
    case I2C0_IC_CON_IC_10BITADDR_SLAVE::_7BITS:
        result = "_7BITS";
        break;
    case I2C0_IC_CON_IC_10BITADDR_SLAVE::_10BITS:
        result = "_10BITS";
        break;
    }

    return result;
}

/**
 * Converts a C string to I2C0_IC_CON_IC_10BITADDR_SLAVE.
 *
 * \param[in]  data   A C string to convert.
 * \param[out] output The enumeration element to write.
 * \return            Whether or not the output was written.
 */
inline bool from_string(const char *data,
                        I2C0_IC_CON_IC_10BITADDR_SLAVE &output)
{
    bool result = false;

    if ((result = !strncmp(data, "_7BITS", 6)))
    {
        output = I2C0_IC_CON_IC_10BITADDR_SLAVE::_7BITS;
    }
    else if ((result = !strncmp(data, "_10BITS", 7)))
    {
        output = I2C0_IC_CON_IC_10BITADDR_SLAVE::_10BITS;
    }

    return result;
}

}; // namespace RP2040
